...
graph TB
    ...
    UC14[Прогнозировать<br/>расходы]
    UC15[Выявить<br/>аномалии]
...
| **OCR текста чека** | При фото чека без QR | google_mlkit_text_recognition |
| **Получить данные из API ФНС** | После распознавания QR | HTTP запрос к proverkacheka.com |
| **Прогнозировать расходы** | При открытии рекомендаций | Prophet (Facebook) |
| **Выявить аномалии** | После каждой транзакции | Isolation Forest |
...
5. После успешной авторизации → переход на главный экран с 29 предустановленными категориями

...
- ML-категоризация расходов с точностью ~88% (Random Forest + TF-IDF)
...
2. **ML категоризация** → Точность ~88% (Random Forest + TF-IDF на 1238 примерах)
...
| **Redis** | 7+ | Кэширование |
| **scikit-learn** | 1.6.1 | ML модели (Random Forest, TF-IDF) |
...
```mermaid
graph TB
    subgraph "ML Pipeline v1.0 — реализовано"
        A[Входные данные] --> B[TF-IDF векторизация]
        B --> C{Тип задачи}
...
- ✅ Высокая точность на табличных данных (~88% на 1238 примерах)
...
```mermaid
flowchart LR
    A[Датасет 1238 транзакций] --> B[Предобработка]
...
TfidfVectorizer(
    ...
    ngram_range=(1, 2),      # Uni и bigrams
)
```

#### Метрики качества (датасет 1238 примеров)

| Метрика | Значение | Описание |
|---------|----------|----------|
| **Accuracy** | 87.90% | Общая точность на тестовой выборке |
| **Precision** | ~90% | Средняя точность по категориям |
| **Recall** | ~88% | Средняя полнота по категориям |
| **F1-Score** | ~88% | Гармоническое среднее |
| **Датасет** | 1238 примеров | 28 категорий |
...
- ✅ ML категоризация (серверная модель, точность 87.90%)
...
- 🔄 **NLP предобработка** (pymorphy2 + NLTK)
  - Лемматизация (приведение к начальной форме слов)
  - Удаление стоп-слов для русского языка
  - Токенизация и нормализация текста
  - **Ожидаемое улучшение:** ML точность 87.90% → 93-95%

- 🔄 **Feature Engineering**
  - Добавление признаков: amount, hour, day_of_week, is_weekend
  - Контекстные признаки: is_morning, is_evening, is_large_purchase
  - **Ожидаемое улучшение:** ML точность 93-95% → 95-97%

- 🔄 **Расширение датасета**
  - Увеличение с 1,238 до 5,000+ примеров
  - Балансировка классов
  - Кэширование результатов ML в Redis (TTL 1 час)

- 🔄 **Встроенная TFLite модель** (планируется v1.2)
  - Офлайн категоризация с целевой точностью 85-87%
  - Размер модели ~10-15 MB
  - Fallback: TFLite (офлайн) → Server ML (онлайн ~88%)
...
**Метрики успеха:**
- Точность ML категоризации: 87.90% → 95-97%
- Точность OCR: 60-70% → 85-95%
- Общая точность системы: ~74% → ~91% (+17%)
